...
describe('linkDensity($)', () => {
  it('returns 0.5 if half of the text is a link', () => {
    const $: cheerio.Root = cheerio.load(HTML.linkDensity5);

    const density: number = linkDensity($('div').first());

    assert.equal(density, 0.5);
  });

  it('returns 1 if all of the text is a link', () => {
    const $: cheerio.Root = cheerio.load(HTML.linkDensity1);
  ...
  it("returns 0 if there's no text", () => {
    const $: cheerio.Root = cheerio.load(HTML.linkDensity0);
